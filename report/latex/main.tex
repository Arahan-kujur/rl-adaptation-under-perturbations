...
Invalid action masking is standard practice in game AI~\citep{huang2022closer}, and constrained MDPs formalise action restrictions~\citep{altman1999constrained}. However, the dynamic consequences of \emph{mid-training} action removal under self-play have not been studied. Our work shows that the strategic impact depends critically on whether the removal eliminates all remaining decisions.
...
The number of reachable information sets at which the perturbed agent retains $>1$ legal action. CAC counts decision points without weighting by reach probability; in larger games, a reach-weighted variant may better capture strategic significance (see Limitations).
...
Cross-game comparison uses $(r - r_{\min}) / (r_{\max} - r_{\min}) \in [0,1]$.

% ===================================================================
\section{Theory}

We formalise the threshold effect in two-player zero-sum extensive-form games. Let $G = (H, P, A, \mathcal{I}, u)$ be such a game with history set $H$, player function $P$, action sets $A$, information partition $\mathcal{I}$, and utility $u$.

\begin{proposition}[Zero-contingency exploitation]
\label{prop:zero}
Let $G'$ be the reduced game obtained by fixing Player~0's action at every information set $I \in \mathcal{I}_0$ (i.e., $\text{CAC}(P_0) = 0$). Then Player~0's value in $G'$ equals the best-response value against $P_0$'s forced policy:
\[
v_0(G') = -\max_{\pi_1 \in \Pi_1} \sum_{z \in Z} u_1(z) \cdot \rho^{\sigma_0^f, \pi_1}(z)
\]
where $\sigma_0^f$ is $P_0$'s forced (deterministic) strategy and $\rho^{\sigma_0^f, \pi_1}(z)$ is the probability of reaching terminal node $z$.
\end{proposition}

\begin{proof}
Under zero contingency, $P_0$ has no strategic choices; $\sigma_0^f$ is the unique strategy available. The game reduces to a single-agent optimisation for $P_1$, whose optimal response is the pure best response $\text{BR}(\sigma_0^f)$. By the minimax theorem applied to the reduced game, this determines $v_0(G')$ uniquely.
\end{proof}

\begin{proposition}[Residual contingency bound]
\label{prop:residual}
If $P_0$ retains at least one information set $h^* \in \mathcal{I}_0$ with $|A(h^*)| \geq 2$, then $P_1$'s best response must condition on $P_0$'s choice at $h^*$. In particular, $P_1$ cannot adopt the pure exploitation strategy of Proposition~\ref{prop:zero}, and:
\[
v_0(G') \geq v_0^{\text{Nash}}(G) - \delta(h^*)
\]
where $\delta(h^*)$ depends on the strategic weight of the forced information sets and vanishes when $h^*$ is reached with high probability.
\end{proposition}

\begin{proof}
At $h^*$, $P_0$ can choose among $|A(h^*)| \geq 2$ actions contingent on $P_1$'s strategy. Any pure $P_1$ strategy that does not account for this choice can be exploited by $P_0$'s adaptation at $h^*$. Therefore $P_1$'s best response must mix or condition on $P_0$'s play at $h^*$, preventing convergence to the zero-contingency exploitation point. The bound follows from the observation that $G'$ contains the strategic interaction at $h^*$ as a subgame.
\end{proof}

\begin{corollary}
The transition from $\text{CAC} = 1$ to $\text{CAC} = 0$ qualitatively changes the best-response structure: from a game requiring mixed or adaptive responses to one with a trivially computable pure best response. This structural discontinuity---not a quantitative reduction---drives the threshold effect.
\end{corollary}
...
All algorithms collapse. DQN reaches $-0.994$---\emph{more} severe than tabular agents. Figure~\ref{fig:dqn-analysis} provides mechanistic insight: after perturbation, DQN's policy entropy drops to near zero and Q-value gaps spike, confirming rapid convergence to a deterministic policy. Function approximation compresses the policy toward extreme action distributions faster than tabular $\varepsilon$-greedy, which retains residual stochasticity at the $\varepsilon$-floor.
...
\paragraph{Liar's Dice: when the ``removed'' action retains strategic value.}
Removing all claims from P0 in Liar's Dice forces challenge-only play. Neither tabular Q-Learning (1-die, 24{,}576 info sets; post $= -0.032$) nor DQN (2-dice, $200{,}000{+}$ info sets; post $= +0.008$) collapses. Unlike forced fold in Kuhn, challenging in Liar's Dice is a strategically meaningful action: it is contingent on P0's private die and the claim history. P0 retains genuine decision-making through the \emph{timing} of challenges, even though the action \emph{type} is fixed. This confirms that the threshold depends on whether strategic flexibility is preserved, not merely on the count of available action types.

\paragraph{Non-zero-sum domains: degradation without collapse.}
In the cooperative Coordination game (match-the-target), forcing P0 to a single action (CAC $= 0$) degrades team performance ($+1.57 \to +1.44$, $p = 0.001$, $d = -3.8$) but does not produce convergence to the DEA. In the Negotiation game (ultimatum, 11 offer actions), forcing P0 to a single offer (CAC $= 0$) degrades outcomes; crucially, P1's policy shifts toward rejection once it detects P0's fixed offer, but this rejection is bounded---P1 does not converge to unconditional rejection as it would under zero-sum exploitation pressure. Retaining partial flexibility (offers 0--2, CAC $= 3$) reverses the degradation entirely. In contrast to competitive settings where zero contingency produces collapse to the DEA, cooperative and mixed-motive environments exhibit bounded degradation, suggesting that the threshold interacts with the underlying interaction structure: zero-sum dynamics amplify the constraint into catastrophic exploitation, while cooperative dynamics produce performance loss without attractor convergence.
...
Figure~\ref{fig:exploit} plots exact exploitability (computed via best-response tree walk) over training. Pre-perturbation, exploitability converges toward zero as the agent approaches Nash. Post-perturbation, exploitability spikes---the agent becomes maximally exploitable under the forced policy. This provides a game-theoretically grounded view of the collapse that complements the raw reward analysis.
...
Games studied range from 1 to $200{,}000{+}$ information sets. The largest game (Liar's Dice 2-dice) uses DQN rather than tabular methods. CAC is unweighted by reach probability. Reversibility may not hold under deeper networks where gradient-based collapse could corrupt representations.

\paragraph{Future work.}
Scaling to larger games; cooperative and general-sum settings; formal exploitability bounds; entropy regularisation as mitigation (preliminary results in Appendix~\ref{app:entropy}); reach-weighted capacity definitions.
